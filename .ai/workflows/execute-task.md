...
5. Load applicable standards — **blocking step, not optional:**
   - Any code change → `.ai/standards/code-conventions.md` (includes comment & ticket ID requirements)
...
8. Implement STEPS. Positive instructions only — constraints are in `exec-context.md`.
   - **All code changes must include detailed comments with the ticket ID** (e.g., `// FMI-916: reason for change`).
9. Run smoke test if in DONE WHEN.
10. Run unit tests if in DONE WHEN.
11. Update docs per DOC UPDATE — **mandatory, not optional:**
    - Update `.ai/skills/{module}.md` if public interface changed.
    - Update `docs/ARCHITECTURE.md` if new patterns/modules introduced.
    - Update `docs/DECISIONS.md` if design decisions were made.
    - Update `docs/LESSONS.md` if pitfalls were discovered.
    - Create/update feature docs if user-facing behavior changed.
...
- [ ] DOC UPDATE completed — all affected docs are accurate and up-to-date
