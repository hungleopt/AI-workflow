...
1. `.ai/AGENTS.md`
1. `docs/CUTOFF.md`
1. `.ai/SKILLS-TODO.md` — check for ❓ rows before starting
1. `.ai/skills/{module}.md` — if exists (skip source scan)
1. `docs/modules/{module}/` — only if in CUTOFF.md AND no skill file
1. `docs/ARCHITECTURE.md` — only if task involves new resource/endpoint/module
...
## Task type splitting (BE / FE / QA)

When a feature spans both backend and frontend, split into **separate task files** per type whenever possible:

| Tag    | Scope                                   | Directories                                                    |
| ------ | --------------------------------------- | -------------------------------------------------------------- |
| `[BE]` | Backend — C#, services, models, API, DB | Everything **except** `firstmile.ui/` and `firstmile.widgets/` |
| `[FE]` | Frontend — JS/TS, CSS, templates, UI    | `firstmile.ui/` and `firstmile.widgets/` only                  |
| `[QA]` | QA verification plan                    | No code changes — `.qa.md` files                               |

**Rules:**

1. Always prefer dedicated BE and FE tasks over a single mixed task.
1. A task must not mix BE and FE file changes — split into two tasks with a dependency.
1. `firstmile.patterns/` is auto-generated — never include it in any task scope.
1. If a feature requires both BE and FE work, create the BE task first (FE typically depends on BE API/model changes).
1. QA tasks (`.qa.md`) are always separate files — they never contain implementation steps.
...
## DIAGRAMS (optional)
<!-- Include Mermaid diagrams when the task involves complex flows, state transitions, or multi-component interactions. Use ```mermaid fenced code blocks. -->
<!-- Examples: sequence diagrams for API calls, flowcharts for decision logic, state diagrams for status transitions. -->
...
- [ ] All code changes include detailed comments with ticket ID (e.g., `// FMI-XXX: reason`)
...
- [ ] Documentation updated: all affected docs (skills, ARCHITECTURE, DECISIONS, LESSONS) reflect current state
...
<!-- Documentation maintenance is MANDATORY — stale docs block future development -->
<!-- Always specify: which doc file + what to add/change + ticket ID for traceability -->
...
1. Append failure/lesson entries → `docs/LESSONS.md`
1. Write one-line summary → `docs/CHANGELOG.md`
...
## Markdown formatting rules (all task files)

1. Every task markdown file MUST start with a clear H1 heading (`#`) in the format `# {NNN} - {BE|FE|QA} - {Task title}` (type tag immediately after the task number).
1. The H1 heading is the first line of the file — no blank line before it.
1. All other markdown headings (`##`, `###`, etc.) must be surrounded by exactly one blank line above and below.
1. Lists and fenced code blocks must be surrounded by exactly one blank line above and below.
1. Markdown files must end with exactly one trailing newline (`\n`).
1. Ordered lists must use `1.` for every item (do not increment numbers manually).
1. Use backtick fenced code blocks with at least three backticks. Extend the fence length (e.g. four backticks) when the content itself includes triple backticks.
1. Do not use indented code blocks.
1. Every fenced code block must declare a language identifier.
1. If a fenced code block is a single line, append `wrap` after the language (e.g. ` ```bash wrap `) so it wraps when rendered on the site.
1. If a fenced code block does not represent an actual programming language, use `text` as the language.
