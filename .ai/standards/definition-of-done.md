...
- [ ] **All code changes include detailed comments explaining WHY + ticket ID** (e.g., `// FMI-916: reason`). See `.ai/standards/code-conventions.md` Comments section.
- [ ] **No anonymous C# types in production code** — all data structures use named models (records, classes, structs). Refactor existing anonymous types when modifying surrounding code.
- [ ] **Documentation is up-to-date** — all affected docs (skill files, ARCHITECTURE.md, DECISIONS.md, LESSONS.md, feature docs) reflect the current state after this change
